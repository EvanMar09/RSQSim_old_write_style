         ...
         double *uzz)
{
  ...
  *ux = *uy = *uz = *uxx = *uyx = *uzx = *uxy = *uyy = *uzy = *uxz = *uyz = *uzz = 0;
  ...
  dccon0(aalpha, ddip, &c0,&c1,&c2);  /* add c0 */
  ...
  xi[0] = x - al1;
  xi[1] = x - al2;
  if (fabs(xi[0]) < eps) xi[0] = 0;
  if (fabs(xi[1]) < eps) xi[1] = 0;
...
*/
  ...
  et[1] = p - aw2;
  if (fabs(q) < eps) q = 0;
  if (fabs(et[0]) < eps) et[0] = 0;
  if (fabs(et[1]) < eps) et[1] = 0;
...
*/
  ...
  for (k=0; k<2; k++)
  {
    for (j=0; j<2; j++)
    {
      dccon2(xi[j], et[k], q, c0.sd, c0.cd, kxi[k], ket[j], &c0, &c1, &c2); /* add all c's */
      ua(xi[j], et[k], q, dd1, dd2, dd3, dua, c0, c1, c2);
      for (i=0; i<=9; i += 3)
      {
        du[i] = -dua[i];
...
*/
  ...
  for (k=0; k<2; k++)
  {
    for (j=0; j<2; j++)
    {
      dccon2(xi[j], et[k], q, c0.sd, c0.cd, kxi[k], ket[j], &c0, &c1, &c2); /* add c's  */                 
      ua(xi[j], et[k], q, dd1, dd2, dd3, dua, c0, c1, c2);                         
      ub(xi[j], et[k], q, dd1, dd2, dd3, dub, c0, c1, c2);                         
      uc(xi[j], et[k], q, zz, dd1, dd2, dd3, duc, c0, c1, c2);                     
...
void ub(double xi, double et, double q, double disl1, double disl2, double disl3, 
        double *u, c0_str c0, c1_str c1, c2_str c2)
{
  double du[12], rd, d11, aj1, aj2, aj3, aj4, aj5, aj6, ai1, ai2, ai3, ai4; 
...
void uc(double xi, double et, double q, double z, double disl1, double disl2, double disl3, 
        double *u, c0_str c0, c1_str c1, c2_str c2)
{
  double du[12], c, x53, y53, z53, h, z32, ppy, ppz, y0, z0;
...
void dccon2(double xi, double et, double q, double sd, double cd, double kxi, double ket, c0_str *c0, c1_str *c1, c2_str *c2)
{
...
void dc3d_passByRef(double *alpha_p, double *x_p, double *y_p, double *z_p,
          ...
          double *uzz)
{
  double alpha, x, y, z, depth, dip, al1, al2, aw1, aw2, disl1, disl2, disl3;
  int err;
  ...
  err = dc3d(alpha, x, y, z, depth, dip, al1, al2, aw1, aw2, disl1, disl2, disl3,
           ...
           double *uzz, int *npts)  
{
  ...
  for (ipt=0; ipt < *npts; ipt++)
    dc3d(*alpha, x[ipt], y[ipt], z[ipt], *depth, *dip, *al1, *al2,
