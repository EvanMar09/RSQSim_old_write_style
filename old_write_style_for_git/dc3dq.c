/* This is nearly a line-for-line translation into C of 
   Okada [1992]'s Fortran code;  done by Keith Richards-Dinger
   in 2007
*/
...
int dc3dq(__float128 alpha, __float128 x, __float128 y, __float128 z,
         ...
         __float128 *uzz)
{
  __float128 xi[2], et[2], kxi[2], ket[2];
  __float128 u[12], du[12], dua[12], dub[12],duc[12];
  __float128 eps=1e-6;
  __float128 aalpha, ddip, zz, dd1, dd2, dd3;
  __float128 d, p, q;
  __float128 r12, r21, r22;
  ...
  /* set output to all zeros, will return this in singular cases */
  *ux = *uy = *uz = *uxx = *uyx = *uzx = *uxy = *uyy = *uzy = *uxz = *uyz = *uzz = 0;
  ...
  dccon0q(aalpha, ddip, &c0,&c1,&c2);  /* add c0 */
  ...
  xi[0] = x - al1;
  xi[1] = x - al2;
  if (fabsq(xi[0]) < eps) xi[0] = 0;
  if (fabsq(xi[1]) < eps) xi[1] = 0;
...
*/
  ...
  et[1] = p - aw2;
  if (fabsq(q) < eps) q = 0;
  if (fabsq(et[0]) < eps) et[0] = 0;
  if (fabsq(et[1]) < eps) et[1] = 0;
...
*/
  ...
  ket[0]=0;                                                         
  ket[1]=0;                                                         
  r12 = sqrtq(xi[0]*xi[0] + et[1]*et[1] + q*q);                          
  r21 = sqrtq(xi[1]*xi[1] + et[0]*et[0] + q*q);                           
  r22 = sqrtq(xi[1]*xi[1] + et[1]*et[1] + q*q);                           
  ...
  for (k=0; k<2; k++)
  {
    for (j=0; j<2; j++)
    {
      dccon2q(xi[j], et[k], q, c0.sd, c0.cd, kxi[k], ket[j], &c0, &c1, &c2); /* add all c's */
      uaq(xi[j], et[k], q, dd1, dd2, dd3, dua, c0, c1, c2);
      for (i=0; i<=9; i += 3)
      {
        du[i] = -dua[i];
...
*/
  ...
  for (k=0; k<2; k++)
  {
    for (j=0; j<2; j++)
    {
      dccon2q(xi[j], et[k], q, c0.sd, c0.cd, kxi[k], ket[j], &c0, &c1, &c2); /* add c's  */                 
      uaq(xi[j], et[k], q, dd1, dd2, dd3, dua, c0, c1, c2);                         
      ubq(xi[j], et[k], q, dd1, dd2, dd3, dub, c0, c1, c2);                         
      ucq(xi[j], et[k], q, zz, dd1, dd2, dd3, duc, c0, c1, c2);                     
...
{
  __float128 pi2 = 6.283185307179586476925286766559005;
  __float128 du[12];
  __float128 xy, qx, qy;
  int i;
...
void ubq(__float128 xi, __float128 et, __float128 q, __float128 disl1, __float128 disl2, __float128 disl3, 
        __float128 *u, c0_str_q c0, c1_str_q c1, c2_str_q c2)
{
  __float128 du[12], rd, d11, aj1, aj2, aj3, aj4, aj5, aj6, ai1, ai2, ai3, ai4; 
  __float128 ak1, ak2, ak3, ak4, x;
  __float128 rd2, xy, qx, qy;
  __float128 pi2 = 6.283185307179586476925286766559005;
  int i;
  ...
  {                                               
    ...
    {                                                         
      x = sqrtq(c2.xi2 + c2.q2);                                             
      ai4 = 1/c0.cdcd*( xi/rd*c0.sdcd +                                    
                      2*atan((et*(x + q*c0.cd) + x*(c2.r + x)*c0.sd)/(xi*(c2.r + x)*c0.cd)) );      
    }                                                         
...
void ucq(__float128 xi, __float128 et, __float128 q, __float128 z, __float128 disl1, __float128 disl2, __float128 disl3, 
        __float128 *u, c0_str_q c0, c1_str_q c1, c2_str_q c2)
{
  __float128 du[12], c, x53, y53, z53, h, z32, ppy, ppz, y0, z0;
  __float128 qq, qx, qy, qr, qqy, qqz, xy;
  __float128 cqx, cdr, yy0;
  __float128 pi2 = 6.283185307179586476925286766559005;
  int i;
...
void dccon2q(__float128 xi, __float128 et, __float128 q, __float128 sd, __float128 cd, __float128 kxi, __float128 ket, c0_str_q *c0, c1_str_q *c1, c2_str_q *c2)
{
...
void dc3d_passByRefq(__float128 *alpha_p, __float128 *x_p, __float128 *y_p, __float128 *z_p,
          ...
          __float128 *uzz)
{
  __float128 alpha, x, y, z, depth, dip, al1, al2, aw1, aw2, disl1, disl2, disl3;
  int err;
  ...
  err = dc3dq(alpha, x, y, z, depth, dip, al1, al2, aw1, aw2, disl1, disl2, disl3,
...
/* dc3d_v is for use from R, so that it can do the loop
   ...
   save all the set up work instead of having dc3d() do it
   each time 


   Args are same as for dc3d_passByRef() except: x, y, and z are
           ...
           __float128 *uzz, int *npts)  
{
  ...
  for (ipt=0; ipt < *npts; ipt++)
    dc3dq(*alpha, x[ipt], y[ipt], z[ipt], *depth, *dip, *al1, *al2,
         *aw1, *aw2, *disl1, *disl2, *disl3, &(ux[ipt]), &(uy[ipt]), (&uz[ipt]),
